...
batch_size = 500 #256
num_epochs = 100  #100
image_size = 32  #32 #72 # We'll resize input images to this size
...
weights_posit_78 = np.float32(model.layers[78].get_weights()[0]);
bias_posit_78 = np.float32(model.layers[78].get_weights()[1])

weights_posit_80 = np.float32(model.layers[80].get_weights()[0])
bias_posit_80 = np.float32(model.layers[80].get_weights()[1])

weights_posit_82 = np.float32(model.layers[82].get_weights()[0])
bias_posit_82 = np.float32(model.layers[82].get_weights()[1])
